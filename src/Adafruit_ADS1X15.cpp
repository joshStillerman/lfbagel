...
void Adafruit_ADS1X15::startADC_SingleEnded(uint8_t channel)
{
  if (channel > 3) return;
...
void Adafruit_ADS1X15::writeRegister(uint8_t reg, uint16_t value)
{
  i2c_reg8_write16(bus, addr, reg, value);
}
...
uint16_t Adafruit_ADS1X15::readRegister(uint8_t reg)
{
  return i2c_reg8_read16(bus, addr, reg);
}
#endif
extern "C" {
void  * Adafruit_ADS1115_new()
{
	return (void *)new Adafruit_ADS1115();
}
void  Adafruit_ADS1115_delete(void * ctx)
{
	delete (Adafruit_ADS1115 *) ctx;
}
void  Adafruit_ADS1115_begin(void *ctx, uint8_t i2c_addr, TwoWire *wire)
{
	((Adafruit_ADS1115 *) ctx)->begin(i2c_addr, wire);
}
void  Adafruit_ADS1115_setGain(void *ctx, adsGain_t gain)
{
	((Adafruit_ADS1115 *) ctx)->setGain(gain);
}
void  Adafruit_ADS1115_startADC_SingleEnded(void *ctx, uint8_t channel)
{
	((Adafruit_ADS1115 *) ctx)->startADC_SingleEnded(channel);
}
float Adafruit_ADS1115_computeVolts(void *ctx, int16_t value)
{
	return ((Adafruit_ADS1115 *) (ctx))->computeVolts(value);
}
int16_t Adafruit_ADS1115_getLastConversionResults(void *ctx)
{
	return ((Adafruit_ADS1115 *) (ctx))->getLastConversionResults();
}
} // extern "C"
