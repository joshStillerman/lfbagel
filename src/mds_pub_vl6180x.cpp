...
static void send_data(double now, float *samples, uint8_t nsamp)
{
    ...
    //MDSplus::EventStream::send(shot, MDSPLUS_EVENT, true, 1, (void *)&inow, 4, samples);
    MDSplus::EventStream::send(shot, MDSPLUS_EVENT, true, 1, (void *)&inow, 1, dims, samples);
}
...
static void send_bogus_data(double now, uint8_t nsamp)
{
    ...
    //                            1, samples);
    MDSplus::EventStream::send(shot, "BOGUS_DATA", true, 1, (void *)&inow, 1, dims, samples);
}
