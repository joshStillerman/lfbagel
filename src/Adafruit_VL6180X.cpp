...
#include <fcntl.h>
#include <sys/ioctl.h>
#else // abstract away the wire interface
#endif
...
boolean Adafruit_VL6180X::begin(TwoWire *theWire)
{
  ...
  _i2c->begin();
#else
boolean Adafruit_VL6180X::begin(int i2c_fd)
{
  _i2c = i2c_fd;
#endif
...
void Adafruit_VL6180X::loadSettings(void) {
  ...
  write8(0x00a3, 0x3c);
  write8(0x00b7, 0x00);
  write8(0x00bb, 0x3c);   /* 0x3c original.. 0x28 for 100 Hz ranging */
  write8(0x00b2, 0x09);
  write8(0x00ca, 0x09);
  write8(0x0198, 0x01);
  write8(0x01b0, 0x17);
  ...
  write8(0x003f, 0x46); // Sets the light and dark gain (upper
                        ...
                        // changed.
  write8(0x0031, 0x00); // sets the # of range measurements after
                        // which auto calibration of system is
                        // performed...  0x00 for disable.  0xFF max.
...
/**
 * @brief Turn on (or off) the self calibration which occurs every 255 samples
 * 
 * @param enable True to turn it on
 */
void Adafruit_VL6180X::enableSelfCalibration(bool enable)
{
  uint8_t calint;

  calint = enable ? 0xFF : 0x00;
  
  write8(0x0031, calint); // sets the # of range measurements after
                            // which auto calibration of system is
                            // performed...  0x00 for disable.  0xFF max.
}
...
uint8_t Adafruit_VL6180X::read8(uint16_t address) {
  ...
  msgs[0].addr = _i2caddr;
  msgs[0].flags = 0;
  msgs[0].len = 2;
  msgs[0].buf = addrbuf;

  msgs[1].addr = _i2caddr;
  ...
  if (ioctl(_i2c, I2C_RDWR, &msgset) < 0) {
...
void Adafruit_VL6180X::write8(uint16_t address, uint8_t data) {
    ...
    if (ioctl(_i2c, I2C_RDWR, &msgset) < 0) {
        perror("ioctl(I2C_RDWR) in write8");
    }
}
